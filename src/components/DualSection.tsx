...
import {
  Server, Cpu, Zap, Droplets, HardDrive,
  Fan, MemoryStick, Plug,
  ChevronRight, X, ArrowRight,
} from "lucide-react";
import { useOrbitResponsive } from "@/hooks/useOrbitResponsive";

const proServices = [
  { name: "Logic Board", icon: Cpu, color: "#f59e0b", details: ["Micro-Soldering", "Chip-Level Repair", "BGA Reballing", "Power IC Fix", "Capacitor Swap"] },
  { name: "Short Circuit", icon: Zap, color: "#ef4444", details: ["Circuit Tracing", "Component Replacement", "MOSFET Repair", "Voltage Regulator Fix"] },
...
const proReplacements = [
  { name: "Storage", icon: HardDrive, color: "#3b82f6", details: ["SSD Upgrade", "NVMe Install", "Data Migration", "RAID Recovery", "Capacity Expansion"] },
  { name: "RAM", icon: MemoryStick, color: "#22c55e", details: ["RAM Upgrade", "Module Replacement", "Slot Repair", "Unified Memory Fix"] },
  { name: "Cooling", icon: Fan, color: "#a855f7", details: ["Fan Replacement", "Thermal Paste Reapply", "Heat Sink Cleaning", "Airflow Restoration"] },
  { name: "PSU", icon: Plug, color: "#ec4899", details: ["Power Supply Swap", "Capacitor Repair", "Voltage Regulator Fix", "Cable Replacement"] },
];
...
const DualSection = () => {
  ...
  const activeSvc = active
    ? active.ring === "service" ? proServices[active.index] : proReplacements[active.index]
    : null;
  ...
  return (
    <section id="pro" ref={ref} className="relative min-h-screen flex flex-col overflow-hidden">
      <motion.div className="absolute inset-0 pointer-events-none" initial={{ opacity: 0 }} animate={isInView ? { opacity: 1 } : {}} transition={{ duration: 1, delay: 0.1 }}>
        <motion.div className="absolute top-1/3 left-1/3 w-[400px] h-[400px] bg-orange-500/[0.03] rounded-full blur-[120px]"
          animate={{ scale: [1, 1.1, 1], x: [0, 20, 0] }} transition={{ duration: 9, repeat: Infinity, ease: "easeInOut" }} />
        <motion.div className="absolute bottom-1/4 right-1/3 w-[350px] h-[350px] bg-rose-500/[0.03] rounded-full blur-[100px]"
      ...
      <div className="relative z-10 text-center pt-8 pb-1 md:pt-12 md:pb-2 flex-shrink-0 px-4">
        <motion.div initial={{ opacity: 0, y: 30, filter: "blur(8px)" }} animate={isInView ? { opacity: 1, y: 0, filter: "blur(0px)" } : {}} transition={{ duration: 0.5, ease: [0.16, 1, 0.3, 1] }}>
          <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-extrabold tracking-tight text-white mb-1 md:mb-2" style={{ fontFamily: "'Inter', 'SF Pro Display', sans-serif" }}>
            Pro Machines{" "}<span className="bg-gradient-to-r from-orange-400 via-rose-400 to-pink-500 bg-clip-text text-transparent">Repairs</span>
          </h2>
        </motion.div>
        <motion.p className="text-xs sm:text-sm text-slate-400 max-w-md mx-auto font-light" style={{ fontFamily: "'Inter', sans-serif" }}
          ...
          Mac Studio & Mac Mini — Inner ring: board-level services · Outer ring: part replacements.
        </motion.p>
      </div>
      ...
      <div className={`relative z-10 flex-1 flex ${isMobile ? "flex-col items-center justify-center py-4 gap-4" : "items-center justify-center"} min-h-0 px-4`}>
        <motion.div className="relative flex-shrink-0" style={{ width: BOX, height: BOX }}
          ...
          <motion.div className="absolute rounded-full" style={{ width: R_OUTER * 2, height: R_OUTER * 2, left: cx - R_OUTER, top: cy - R_OUTER, border: "1px solid rgba(251,113,133,0.12)" }}
          ...
          <motion.div className="absolute rounded-full" style={{ width: R_INNER * 2, height: R_INNER * 2, left: cx - R_INNER, top: cy - R_INNER, border: "1px solid rgba(249,115,22,0.18)" }}
          ...
          <motion.svg className="absolute pointer-events-none"
            ...
            <defs>
              <path id="pro-inner" d={`M ${R_OUTER + 15 * scale}, ${R_OUTER + 15 * scale - R_INNER} A ${R_INNER},${R_INNER} 0 1,1 ${R_OUTER + 15 * scale - 0.01}, ${R_OUTER + 15 * scale - R_INNER}`} />
              <path id="pro-outer" d={`M ${R_OUTER + 15 * scale}, ${15 * scale} A ${R_OUTER},${R_OUTER} 0 1,1 ${R_OUTER + 15 * scale - 0.01}, ${15 * scale}`} />
            </defs>
            <text><textPath href="#pro-inner" startOffset="5%" style={{ fill: "rgba(249,115,22,0.5)", fontSize: `${Math.max(6, 9 * scale)}px`, fontWeight: 700, letterSpacing: "0.35em", fontFamily: "'Inter', sans-serif" }}>S · E · R · V · I · C · E · S</textPath></text>
            <text><textPath href="#pro-outer" startOffset="2%" style={{ fill: "rgba(251,113,133,0.45)", fontSize: `${Math.max(6, 9 * scale)}px`, fontWeight: 700, letterSpacing: "0.3em", fontFamily: "'Inter', sans-serif" }}>R · E · P · L · A · C · E · M · E · N · T · S</textPath></text>
          </motion.svg>

          <motion.div className="absolute rounded-full pointer-events-none"
            style={{
              width: R_OUTER * 2, height: R_OUTER * 2, left: cx - R_OUTER, top: cy - R_OUTER,
              background: "conic-gradient(from 0deg, transparent 0%, rgba(249,115,22,0.06) 18%, transparent 35%, rgba(251,113,133,0.05) 60%, transparent 80%, rgba(236,72,153,0.04) 92%, transparent 100%)"
            }}
          ...
          {/* Pro Machine center */}
          <div className="absolute z-10" style={{ left: cx - 35 * scale, top: cy - 22 * scale }}>
            <motion.div initial={{ opacity: 0, y: 35, scale: 0.8, filter: "blur(6px)" }} animate={isInView ? { opacity: 1, y: 0, scale: 1, filter: "blur(0px)" } : {}}
              ...
              <motion.div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full blur-[35px]"
                style={{ width: 96 * scale, height: 96 * scale, backgroundColor: "rgba(249,115,22,0.1)" }}
              ...
              <motion.div animate={isInView ? { y: [0, -5, 0] } : {}} transition={{ duration: 5, repeat: Infinity, ease: "easeInOut", delay: 0.8 }} className="relative">
                <div style={{ width: 70 * scale, height: 44 * scale, borderRadius: 8 * scale }}
                  className="bg-gradient-to-b from-slate-800 to-slate-900 border-2 border-white/[0.12] shadow-[0_0_30px_rgba(249,115,22,0.1)] overflow-hidden relative flex flex-col items-center justify-center">
                  <Server style={{ width: 14 * scale, height: 14 * scale }} className="text-orange-400/40" strokeWidth={1} />
                  <span style={{ fontSize: Math.max(4, 5 * scale), fontFamily: "'Inter', sans-serif" }}
                    className="font-semibold text-white/40 tracking-[0.1em] uppercase mt-0.5">Mac Pro</span>
                  <AnimatePresence>
                    {activeSvc && (
          ...
          {proServices.map((svc, i) => {
            const { x, y } = getCirclePos(i, proServices.length, R_INNER);
          ...
          {proReplacements.map((rep, i) => {
            const { x, y } = getCirclePos(i, proReplacements.length, R_OUTER);
            ...
            <motion.div key={`panel-${activeSvc.name}`}
              ...
              <div className="w-full rounded-3xl bg-slate-900/70 backdrop-blur-2xl border border-white/[0.08] p-5 sm:p-6 shadow-2xl relative" style={{ borderColor: `${activeSvc.color}20` }}>
                ...
                <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-[9px] uppercase tracking-[0.2em] font-semibold mb-3 block"
                  style={{ color: active!.ring === "service" ? "#fb923c" : "#fb7185" }}>
