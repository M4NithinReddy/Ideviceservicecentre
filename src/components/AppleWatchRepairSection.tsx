...
import {
    Watch, Fingerprint, Monitor, BatteryFull,
    CircleDot, RectangleHorizontal, Droplets, Wrench,
...
const AppleWatchRepairSection = () => {
    ...
    const activeSvc = active
        ? active.ring === "service" ? watchServices[active.index] : watchReplacements[active.index]
        : null;
    ...
    const iconS = Math.max(10, 13 * scale);
    const iconR = Math.max(11, 14 * scale);
    const labelS = Math.max(4, 5 * scale);
    ...
    return (
        <section id="watch" ref={ref} className="relative min-h-screen flex flex-col overflow-hidden md:overflow-visible">
            <motion.div className="absolute inset-0 pointer-events-none" initial={{ opacity: 0 }} animate={isInView ? { opacity: 1 } : {}} transition={{ duration: 1, delay: 0.1 }}>
                ...
                <motion.div className="absolute bottom-1/4 left-1/3 w-[300px] h-[300px] bg-amber-500/[0.03] rounded-full blur-[100px]"
            ...
            <div className="relative z-10 text-center pt-8 pb-1 md:pt-12 md:pb-2 flex-shrink-0 px-4">
                <motion.div initial={{ opacity: 0, y: 30, filter: "blur(8px)" }} animate={isInView ? { opacity: 1, y: 0, filter: "blur(0px)" } : {}} transition={{ duration: 0.5, ease: [0.16, 1, 0.3, 1] }}>
                    <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-extrabold tracking-tight text-white mb-1 md:mb-2" style={{ fontFamily: "'Inter', 'SF Pro Display', sans-serif" }}>
                        Apple Watch{" "}<span className="bg-gradient-to-r from-rose-400 via-orange-400 to-amber-500 bg-clip-text text-transparent">Repairs</span>
                    </h2>
                </motion.div>
                <motion.p className="text-xs sm:text-sm text-slate-400 max-w-md mx-auto font-light" style={{ fontFamily: "'Inter', sans-serif" }}
            ...
            <div className={`relative z-10 flex-1 flex ${isMobile ? "flex-col items-center justify-start py-8 gap-8 overflow-y-visible" : "items-center justify-center"} min-h-0 px-4`}>
                ...
                {isMobile ? (
                    ...
                    <div className="w-full max-w-md space-y-10 pb-10">
                        ...
                        >
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                {watchServices.map((svc, i) => {
                                    const isActive = active?.ring === "service" && active.index === i;
                                    return (
                                        <button
                                            key={`mob-svc-${svc.name}`}
                                            onClick={() => handleClick("service", i)}
                                            className={`flex flex-col items-center justify-center p-5 rounded-3xl border transition-all duration-300 active:scale-95 ${isActive ? "bg-slate-800 border-rose-500/50 shadow-[0_0_20px_rgba(244,63,94,0.15)]" : "bg-slate-900/60 border-white/5"}`}
                                        >
                        ...
                        >
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                {watchReplacements.map((rep, i) => {
                                    const isActive = active?.ring === "replacement" && active.index === i;
                                    return (
                                        <button
                                            key={`mob-rep-${rep.name}`}
                                            onClick={() => handleClick("replacement", i)}
                                            className={`flex flex-col items-center justify-center p-5 rounded-3xl border transition-all duration-300 active:scale-95 ${isActive ? "bg-slate-800 border-amber-500/50 shadow-[0_0_20px_rgba(251,191,36,0.15)]" : "bg-slate-900/60 border-white/5"}`}
                                        >
                    ...
                    <motion.div className="relative flex-shrink-0" style={{ width: BOX, height: BOX }}
                        animate={{ x: !isMobile && isOpen ? -140 : 0 }} transition={{ duration: 0.4, ease: [0.32, 0.72, 0, 1] }}>

                        <motion.div className="absolute rounded-full" style={{ width: R_OUTER * 2, height: R_OUTER * 2, left: cx - R_OUTER, top: cy - R_OUTER, border: "1px solid rgba(251,191,36,0.12)" }}
                        ...
                        <motion.div className="absolute rounded-full" style={{ width: R_INNER * 2, height: R_INNER * 2, left: cx - R_INNER, top: cy - R_INNER, border: "1px solid rgba(244,63,94,0.18)" }}
                        ...
                        <motion.svg className="absolute pointer-events-none"
                            ...
                            <defs>
                                <path id="w-inner" d={`M ${R_OUTER + 15 * scale}, ${R_OUTER + 15 * scale - R_INNER} A ${R_INNER},${R_INNER} 0 1,1 ${R_OUTER + 15 * scale - 0.01}, ${R_OUTER + 15 * scale - R_INNER}`} />
                                <path id="w-outer" d={`M ${R_OUTER + 15 * scale}, ${15 * scale} A ${R_OUTER},${R_OUTER} 0 1,1 ${R_OUTER + 15 * scale - 0.01}, ${15 * scale}`} />
                            </defs>
                            <text><textPath href="#w-inner" startOffset="5%" style={{ fill: "rgba(244,63,94,0.5)", fontSize: `${Math.max(6, 9 * scale)}px`, fontWeight: 700, letterSpacing: "0.35em", fontFamily: "'Inter', sans-serif" }}>S · E · R · V · I · C · E · S</textPath></text>
                            <text><textPath href="#w-outer" startOffset="2%" style={{ fill: "rgba(251,191,36,0.45)", fontSize: `${Math.max(6, 9 * scale)}px`, fontWeight: 700, letterSpacing: "0.3em", fontFamily: "'Inter', sans-serif" }}>R · E · P · L · A · C · E · M · E · N · T · S</textPath></text>
                        </motion.svg>

                        <motion.div className="absolute rounded-full pointer-events-none"
                            style={{
                                width: R_OUTER * 2, height: R_OUTER * 2, left: cx - R_OUTER, top: cy - R_OUTER,
                                background: "conic-gradient(from 0deg, transparent 0%, rgba(244,63,94,0.06) 18%, transparent 35%, rgba(251,191,36,0.05) 60%, transparent 80%, rgba(249,115,22,0.04) 92%, transparent 100%)"
                            }}
                        ...
                        {/* Watch center */}
                        <div className="absolute z-10" style={{ left: cx - 36 * scale, top: cy - 44 * scale }}>
                            <motion.div initial={{ opacity: 0, y: 35, scale: 0.8, filter: "blur(6px)" }} animate={isInView ? { opacity: 1, y: 0, scale: 1, filter: "blur(0px)" } : {}}
                                ...
                                <motion.div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full blur-[35px]"
                                    style={{ width: 96 * scale, height: 96 * scale, backgroundColor: "rgba(244,63,94,0.1)" }}
                                    animate={{ scale: [1, 1.2, 1], opacity: [0.3, 0.5, 0.3] }} transition={{ duration: 4, repeat: Infinity, ease: "easeInOut", delay: 0.5 }} />
                                <motion.div animate={isInView ? { y: [0, -5, 0] } : {}} transition={{ duration: 5, repeat: Infinity, ease: "easeInOut", delay: 0.8 }} className="relative">
                                    <div className="flex flex-col items-center">
                                        <div style={{ width: 36 * scale, height: 10 * scale, borderRadius: `${4 * scale}px ${4 * scale}px 0 0` }}
                                            className="bg-gradient-to-b from-slate-700 to-slate-800 border border-b-0 border-white/[0.08]" />
                                        <div style={{ width: 72 * scale, height: 68 * scale, borderRadius: 16 * scale }}
                                            className="bg-gradient-to-b from-slate-800 to-slate-900 border-2 border-white/[0.12] shadow-[0_0_30px_rgba(244,63,94,0.1)] overflow-hidden relative">
                                            <div style={{ inset: 3 * scale, borderRadius: 11 * scale }}
                                                ...
                                                <Watch style={{ width: 14 * scale, height: 14 * scale }} className="text-rose-400/40" strokeWidth={1} />
                                                <span style={{ fontSize: Math.max(4, 6 * scale), fontFamily: "'Inter', sans-serif" }}
                                                    className="font-semibold text-white/40 tracking-[0.1em] uppercase mt-0.5">Watch</span>
                                                <AnimatePresence>
                                                    {activeSvc && (
                                                        <motion.div key={activeSvc.name} initial={{ opacity: 0, y: 3 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -3 }} className="mt-0.5 text-center">
                                                            <span style={{ fontSize: Math.max(3, 4 * scale), color: activeSvc.color }} className="font-bold">{activeSvc.name}</span>
                                                        </motion.div>
                                                    )}
                                                </AnimatePresence>
                                            </div>
                                            <div className="absolute -right-[3px] bg-slate-700 rounded-r-sm border border-l-0 border-white/10"
                                                style={{ top: 12 * scale, width: 4 * scale, height: 10 * scale }} />
                                        </div>
                                        <div style={{ width: 36 * scale, height: 10 * scale, borderRadius: `0 0 ${4 * scale}px ${4 * scale}px` }}
                                            className="bg-gradient-to-b from-slate-800 to-slate-700 border border-t-0 border-white/[0.08]" />
                                    </div>
                                </motion.div>
                            </motion.div>
                        </div>
                        ...
                        {watchServices.map((svc, i) => {
                            const { x, y } = getCirclePos(i, watchServices.length, R_INNER);
                            const isActive = active?.ring === "service" && active.index === i;
                            return (
                                <motion.button key={`svc-${svc.name}`} initial={{ opacity: 0, scale: 0 }} animate={isInView ? { opacity: 1, scale: 1 } : {}}
                        ...
                        {watchReplacements.map((rep, i) => {
                            const { x, y } = getCirclePos(i, watchReplacements.length, R_OUTER);
                            const isActive = active?.ring === "replacement" && active.index === i;
                            return (
                                <motion.button key={`rep-${rep.name}`} initial={{ opacity: 0, scale: 0 }} animate={isInView ? { opacity: 1, scale: 1 } : {}}
                                    transition={{ duration: 0.5, delay: 0.45 + i * 0.05, type: "spring", bounce: 0.25 }}
                                    onClick={() => handleClick("replacement", i)}
                                    className={`absolute z-20 rounded-full backdrop-blur-xl flex flex-col items-center justify-center cursor-pointer transition-all duration-300 border ${isActive ? "bg-slate-800" : "bg-slate-900/80 border-white/10 hover:border-white/20 hover:bg-slate-800/80"}`}
                                    style={{ width: NODE_R, height: NODE_R, left: cx + x - NODE_R / 2, top: cy + y - NODE_R / 2, borderColor: isActive ? rep.color : undefined, boxShadow: isActive ? `0 0 20px ${rep.color}30` : "0 0 10px rgba(0,0,0,0.3)" }}
                                    whileHover={{ scale: 1.15, boxShadow: `0 0 20px ${rep.color}20`, transition: { duration: 0.2 } }} whileTap={{ scale: 0.92 }}>
                                    {isActive && <motion.div className="absolute inset-0 rounded-full border" style={{ borderColor: `${rep.color}35` }} animate={{ scale: [1, 1.4, 1], opacity: [0.5, 0, 0.5] }} transition={{ duration: 2, repeat: Infinity }} />}
                                    <rep.icon style={{ color: isActive ? rep.color : "#94a3b8", width: iconR, height: iconR }} strokeWidth={1.5} />
                                    <span style={{ fontSize: labelR, color: isActive ? rep.color : "#cbd5e1", fontFamily: "'Inter', sans-serif" }} className="font-bold tracking-wide leading-none mt-0.5">{rep.name}</span>
                                </motion.button>
                            );
                        })}
                    </motion.div>
                        ...
                        <motion.div key={`panel-${activeSvc.name}`}
                            ...
                            <div className="w-full rounded-3xl bg-slate-900/70 backdrop-blur-2xl border border-white/[0.08] p-5 sm:p-6 shadow-2xl relative" style={{ borderColor: `${activeSvc.color}20` }}>
                                ...
                                <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-[9px] uppercase tracking-[0.2em] font-semibold mb-3 block"
                                    style={{ color: active!.ring === "service" ? "#fb7185" : "#fbbf24" }}>
...
export default AppleWatchRepairSection;
